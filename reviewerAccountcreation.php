    ...
    if ($userIdResult && $userIdResult->num_rows > 0) {
        ...
        if ($roleIdResult && $roleIdResult->num_rows > 0) {
            ...
            if ($insertRoleStmt->execute()) {
                ...
                $insertProfileStmt = $conn->prepare("INSERT INTO reviewer_profiles (user_id, designation, first_name, last_name, middle_initial, suffix) VALUES (?, ?, ?, ?, ?, ?)");
                $insertProfileStmt->bind_param("isssss", $userId, $designation, $firstName, $lastName, $middleInitial, $suffix);

                if ($insertProfileStmt->execute()) {
                    $success = 'Account created successfully! Wait for the account activation.';
                    // Unset session variables
                    unset($_SESSION['email'], $_SESSION['password'], $_SESSION['role'], $_SESSION['first_name'], $_SESSION['last_name'], $_SESSION['middle_initial'], $_SESSION['suffix'], $_SESSION['designation']);
                } else {
