...
$query = "
SELECT 
    rt.id AS researcher_title_id,
    rt.uploaded_at AS title_uploaded_at,
    rt.study_protocol_title,
    rt.research_category,
    rt.college,
    rt.adviser_name,
    rt.email,
    rt.mobile_number,
    ...
    GROUP_CONCAT(DISTINCT CONCAT(ri.first_name, ' ', ri.middle_initial, ' ', ri.last_name, ' ', COALESCE(ri.suffix, '')) SEPARATOR ', ') 
    AS researchers_involved,

    GROUP_CONCAT(DISTINCT rf.filename SEPARATOR ', ') AS submitted_files

FROM ResearcherTitleInfo_NoUser rt
LEFT JOIN ResearcherInvolved_NoUser ri ON rt.id = ri.researcher_title_id
LEFT JOIN researcher_filesnouser rf ON rt.id = rf.researcher_title_id
GROUP BY rt.id
...
$query = "SELECT DISTINCT college, research_category, status FROM ResearcherTitleInfo_nouser";
...
$query = "SELECT DISTINCT DATE_FORMAT(uploaded_at, '%M %Y') AS month_year FROM ResearcherTitleInfo_nouser ORDER BY uploaded_at DESC";
...
$statusOptions = [];
$query = "SELECT DISTINCT status FROM ResearcherTitleInfo_NoUser";
$resultStatus = $conn->query($query);
...
.restitle {
          ...
          .researcher-group { margin-bottom: 10px; }
        .remove-btn { color: red; cursor: pointer; margin-left: 10px; }
        #formContainer { display: none; border: 1px solid #ccc; padding: 15px; margin-top: 10px; }
...
<body>
  ...
  <button id="showFormBtn">Insert Data</button>

<!-- Hidden form container -->
<div id="formContainer">
    <h2>Insert Researcher Data</h2>
    <form id="researchForm" enctype="multipart/form-data">
        <!-- Study Protocol Title -->
        <label>Study Protocol Title:</label>
        <input type="text" name="study_protocol_title" required><br><br>
...
<input type="text" name="mobile_number" pattern="^(\+63|0)9\d{9}$" placeholder="09XXXXXXXXX or +639XXXXXXXXX" required>
        ...
        <!-- Dynamic Researcher Input Fields -->
        <label>Researchers:</label>
        <div id="researchers">
            <div class="researcher-group">
                <input type="text" name="first_name[]" placeholder="First Name" required>
                <input type="text" name="last_name[]" placeholder="Last Name" required>
                <input type="text" name="middle_initial[]" placeholder="Middle Initial (Optional)" maxlength="2">
                <span class="remove-btn" onclick="removeResearcher(this)">❌</span>
            </div>
        </div>
        <button type="button" onclick="addResearcher()">Add More</button><br><br>

        <!-- College Dropdown (Dynamically Loaded via AJAX) -->
        <label>College:</label>
        <select name="college" id="collegeDropdown" required>
            <option value="">Select College</option>
        </select><br><br>

        <!-- Adviser Name -->
        <label>Adviser Name:</label>
        <input type="text" name="adviser_name" required><br><br>

        <label>Application Form for Research Ethics Review (PDF with Signature):</label>
<input type="file" name="application_form" accept=".pdf" required><br><br>

<label>Research Protocol/Proposal (PDF with Page & Line Number):</label>
<input type="file" name="research_protocol" accept=".pdf" required><br><br>

<label for="tech_review">Technical Review Clearance (PDF only):</label>
<input type="file" name="technical_review_clearance" id="tech_review" accept=".pdf" required><br><br>

<label>Data Collection Instruments (PDF with Page & Line Number):</label>
<input type="file" name="data_collection" accept=".pdf" required><br><br>

<label>Informed Consent/Assent (PDF with Page & Line Number):</label>
<input type="file" name="informed_consent" accept=".pdf" required><br><br>

<label>Curriculum Vitae of Researcher/s (PDF):</label>
<input type="file" name="cv_researcher" accept=".pdf" required><br><br>

<label>Completed Study Protocol Assessment Form (DOCX):</label>
<input type="file" name="study_protocol_form" accept=".doc,.docx" required><br><br>

<label>Completed Informed Consent Assessment Form (DOCX):</label>
<input type="file" name="informed_consent_form" accept=".doc,.docx" required><br><br>

<label>Completed Exempt Review Assessment Form (DOCX):</label>
<input type="file" name="exempt_review_form" accept=".doc,.docx" required><br><br>

    <label>Other Documents (NCIP Clearance, MOA, MOU, etc.) (PDF only with researcher/s signature):</label>
    <div id="otherDocsContainer">
        <div class="other-doc-group">
            <input type="file" name="other_docs[]" accept=".pdf" required>
            <span class="remove-btn" onclick="removeOtherDoc(this)">❌</span>
        </div>
    </div>
    <button type="button" id="addOtherDocBtn">Add More</button>
        

        <!-- Submit and Cancel Buttons -->
        <div class="btn-group">
            <button type="submit">Submit</button>
            <button type="button" id="cancelFormBtn" style="background: red; color: white;">Cancel</button>
        </div>
    </form>
</div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>" . htmlspecialchars($row['title_uploaded_at']) . "</td>";
        echo "<td><button class='view-btn' data-id='" . $row['researcher_title_id'] . "'>View</button></td>"; // Use id field for data-id
        ...
        echo "<td><button class='view-files-btn' data-id='" . $row['researcher_title_id'] . "'>View Files</button></td>"; // Now using rti.id
        echo "<td class='emailColumn'>" . htmlspecialchars($row['email']) . "</td>";

        echo "<td>" . htmlspecialchars($row['mobile_number']) . "</td>";
        echo "<td>
        <select class='status-dropdown' data-id='" . $row['researcher_title_id'] . "'>
        ...
        <input type='text' class='status-input' data-id='" . $row['researcher_title_id'] . "' placeholder='Enter custom status' style='display:none;'>
      </td>";
        ...
        echo "<td>
                <select class='type-review-dropdown' data-id='" . $row['researcher_title_id'] . "'>
              ...
              echo "<td>
              <button class='toggle-btn' data-id='" . $row['researcher_title_id'] . "' data-toggle='" . $row['Toggle'] . "'>" . 
              ($row['Toggle'] == 1 ? "Exclude" : "Include") . 
              "</button>
            </td>";
              
         // Generate button
         echo '<td><button class="generate-btn" data-id="' . $row['researcher_title_id'] . '" data-review-type="' . htmlspecialchars($row['type_of_review']) . '">Generate</button></td>';

         // Add the Edit button in each row
         echo "<td><button class='edit-btn' data-id='" . $row['researcher_title_id'] . "'>Edit</button></td>";
  ...
  <!-- partial -->
    ...
    document.querySelectorAll('.view-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch('fetch_researchersnouser.php', {
                method: 'POST',
                headers: {
...
document.querySelectorAll('.view-files-btn').forEach(button => {
    ...
    $('.type-review-dropdown').change(function() {
        ...
        $.ajax({
            url: 'update_type_reviewNouser.php', // Replace with your PHP script to handle the update
            type: 'POST',
...
document.querySelectorAll('.generate-btn').forEach(button => {
    button.addEventListener('click', function () {
        ...
        fetch(`check_certificate_statusNouser.php?rti_id=${rtiId}`, {
            method: 'GET',
        })
        .then(response => response.json())
        .then(data => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (reviewType === 'Exempt') {
                            certEndpoint = `generate_cert_exemptNouser.php?user_id=${userId}&date=${selectedDate}`;
                            coverLetterEndpoint = `generate_cover_letter_exemptNouser.php?user_id=${userId}&date=${selectedDate}`;
                        } else if (reviewType === 'Full Review' || reviewType === 'Expedited') {
                            certEndpoint = `generate_REC_FLorEXPNouser.php?user_id=${userId}&date=${selectedDate}`;
                            coverLetterEndpoint = `generate_cover_letter_researchEthicsNouser.php?user_id=${userId}&date=${selectedDate}`;
                        } else {
        ...
        .then(data => {
            if (data.trim() === "success") {
                Swal.fire({
                    ...
                    window.location.href = "admin_insertedapplicationforms.php"; // Refresh page
                });
            } else {
                Swal.fire({
                    title: "Error!",
...
let status = statusDropdown ? statusDropdown.value.toLowerCase().trim() : "";
        ...
        if (matchesTitle && matchesEmail && matchesCollege && matchesCategory && matchesMonthYear && matchesStatus) {
            row.style.display = "";
        } else {
 ...
 document.getElementById("showFormBtn").addEventListener("click", function() {
            ...
            this.style.display = "none"; // Hide the "Insert Data" button
        });

        // Hide form when "Cancel" button is clicked and show "Insert Data" button again
        document.getElementById("cancelFormBtn").addEventListener("click", function() {
            document.getElementById("formContainer").style.display = "none";
            document.getElementById("showFormBtn").style.display = "block";
        });

        // Function to add a new researcher input field
        function addResearcher() {
            let researcherDiv = document.createElement('div');
            researcherDiv.className = "researcher-group";
            researcherDiv.innerHTML = `
                ...
                <span class="remove-btn" onclick="removeResearcher(this)">❌</span>
            `;
            document.getElementById('researchers').appendChild(researcherDiv);
        }

        // Function to remove a researcher input field
        function removeResearcher(element) {
    ...
    $.ajax({
        ...
        success: function(data){
            $("#collegeDropdown").append(data);
        }
    });

    document.getElementById("addOtherDocBtn").addEventListener("click", function() {
    let docDiv = document.createElement("div");
    docDiv.className = "other-doc-group";
    docDiv.innerHTML = `
        <input type="file" name="other_docs[]" accept=".pdf" required>
        <span class="remove-btn" onclick="removeOtherDoc(this)">❌</span>
    `;
    document.getElementById("otherDocsContainer").appendChild(docDiv);
});
        ...
        $.ajax({
            ...
            success: function(response){
                Swal.fire({
                    ...
                    if (result.isConfirmed && response.includes("successfully")) {
                        $("#researchForm")[0].reset();  // Clear form after success
                        $("#formContainer").hide();  // Hide form after submission
                        $("#showFormBtn").show();  // Show the "Insert Data" button again
                    }
                });
            }
        });
...
function removeOtherDoc(element) {
    element.parentElement.remove();
}
$(document).ready(function () {
