                    ...
                    <tr>
                        ...
                        <td>
                            <select class='status-dropdown' data-id='<?= clean($data['id']) ?> '>
                                <option value=' <?= clean($data['status']) ?>' selected>
                                    <?= ($data['status'] == 'Notifying') ? 'Issuance of Certificate' : clean($data['status'])  ?>
                                </option>
                            ...
                            <?php if($data['status'] == "Issuance of Certificate"){
                                ...
                                if($data['status'] == "Notifying"){
                                 ...
                                 ?>
                            <input type='text' class='status-input' data-id='<?= clean($data['id']) ?>'
                                placeholder='Enter custom status' style='display:none;'>
                        </td>
                        <td>
                            <select class='type-review-dropdown' data-id='<?= clean(clean($data['id'])) ?> '>
                                <option value='Full Review'
                                    <?=  clean($data['type_of_review']) === 'Full Review' ? 'selected' : ''   ?>>Full
                                    Review</option>
                                <option value='Expedited'
                                    <?=  clean($data['type_of_review']) === 'Expedited' ? 'selected' : ''   ?>>Expedited
                                </option>
                                <option value='Exempt'
                                    <?=  clean($data['type_of_review']) === 'Exempt' ? 'selected' : ''   ?>>Exempt
                                </option>
                            </select>
                        </td>
                        <td><button class="generate-recommendation"><a
                                    href="RC_letter.php?id=<?= $data['id'] ?>">Create</a> </button></td>
                        <td>
                        ...
                        <td><button class="generate-btn" data-id="<?= $data['id']  ?>"
                                data-review-type="<?= clean($data['type_of_review'])  ?>">Generate</button></td>
                        <td><button class='edit-btn'><a
                                    href=' <?=' /REOC/editResearch.php?id='. $data['id'] ?>'>EDIT</a></button></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
    ...
    document.querySelectorAll('.view-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch('/REOC/fetch_researchers.php', {
                    ...
                    body: JSON.stringify({
                        researcher_title_id: researcherTitleId
                    }) // Send the researcher_title_id to the server
                })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        data.researchers.forEach(researcher => {
                            researcherDetails +=
                                `<p><strong>Name:</strong> ${researcher.first_name} ${researcher.middle_initial}. ${researcher.last_name} ${researcher.suffix ? researcher.suffix : ''}</p>`;
                        });
    ...
    document.querySelectorAll('.view-files-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch('/REOC/fetch_files.php', {
                    ...
                    body: JSON.stringify({
                        researcher_title_id: researcherTitleId
                    }) // Send researcher_title_id instead of user_id
                })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        data.files.forEach(file => {
                            fileDetails +=
                                `<p><strong>Type:</strong> ${file.file_type} <br> <strong>Filename:</strong> <a href="${file.file_path}" target="_blank">${file.filename}</a></p>`;
                        });
                        ...
                        Swal.fire({
                            title: 'Error',
                            text: 'No files found for this researcher title.',
                            icon: 'error'
                        });
                    }
                })
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                if (reviewType === 'Exempt') {
                                    certEndpoint =
                                        `/REOC/generate_cert_exempt.php?user_id=${userId}&date=${selectedDate}`;
                                    coverLetterEndpoint =
                                        `/REOC/generate_cover_letter_exempt.php?user_id=${userId}&date=${selectedDate}`;
                                } else if (reviewType === 'Full Review' ||
                                    reviewType === 'Expedited') {
                                    certEndpoint =
                                        `/REOC/generate_REC_FLorEXP.php?user_id=${userId}&date=${selectedDate}`;
                                    coverLetterEndpoint =
                                        `/REOC/generate_cover_letter_researchEthics.php?user_id=${userId}&date=${selectedDate}`;
                                } else {
                                            ...
                                            if (coverLetterEndpoint) {
                                                fetch(coverLetterEndpoint, {
                                                    ...
                                                    .catch(error => {
                                                        Swal.fire({
                                                            title: 'Error',
                                                            text: 'Error generating cover letter: ' +
                                                                error
                                                                .message,
                                                            icon: 'error',
                                                        });
